...
pico.unpack_cascade = function(bytes) {
  ...
  /* we skip the first 8 bytes of the cascade file (cascade version number and some data used during the learning process) */
  let p = 8;
  ...
  function classify_region(r, c, s, pixels, ldim) {
    ...
    for (let i = 0; i < ntrees; ++i) {
      idx = 1;
      // we use '>> 8' here to perform an integer division: this seems important for
...
pico.instantiate_detection_memory = function(size) {
  ...
  /* build a function that: (1) inserts the current frame's detections into the buffer; (2) merges all detections from the last `size` frames and returns them */
...
pico.rgba_to_grayscale = function(rgba, nrows, ncols) {
...
pico.download_face_detection_cascade = function(callback) {
  fetch("/face/facefinder").then(function(response) {
    response.arrayBuffer().then(function(buffer) {
      let bytes = new Int8Array(buffer);
      let classify_region = pico.unpack_cascade(bytes);
      callback(classify_region);
    });
  });
};
